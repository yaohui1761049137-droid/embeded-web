...
NIC_PANEL_MARKERS = (
    ...
    ('data-win="60"', "1-minute range tab missing"),
    ('onclick="setMonWindow(60)">近 1 分钟</a>', "1-minute tab label missing"),
    ('<a class="board-tab active" data-win="60"',
     "1-minute tab is not the default (active)"),
    ("峰值（条/秒）", "peak card unit must be 条/秒"),
    ("function fmtPerSec", "per-second rate formatter missing"),
    ("function fmtClockSec", "second-level tick formatter missing"),
    ("function fmtTsSec", "second-level tooltip timestamp missing"),
    ("nicInfo.ts - an + 1", "1m view does not slice the daemon per-second ring"),
    ("function schedNtpMon", "adaptive stats poller missing"),
    ("var monWindow = 60;", "default monWindow must be 60 (1 minute)"),
    ("var monWinSfx = '1m';", "default monWinSfx must be 1m"),
