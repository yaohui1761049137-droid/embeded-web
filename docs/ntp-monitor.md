...
| 请求量趋势（近 24h，页级时间范围 1m/1h/5h/24h 可切） | `chronyc -c serverstats` 的 `ntp_hits`（每分钟采样） | 折线 = 每分钟请求数（差分）；浅色线 = 累计值（右轴）。chronyd 重启使计数器归零 → 曲线断点。峰值卡单位为**条/秒**（每分钟峰值 ÷ 60，对所有范围生效） |
| 各网口请求量（柱状图） | iptables 每网口 `udp/123` 计数（每分钟采样） | 精确到网口、**含被 ACL 拒绝的请求**；累计值（规则生效以来）+ 近 1m/1h/5h/24h 增量（tooltip 随页级窗口切换）。设备重启后清零 |
...
- 图 1「请求量趋势」：单主轴=每分钟请求数（面积渐变）；悬停十字线 + tooltip 显示
  时间 / 每分钟 / 累计；顶部**数字卡**＝当前速率 · 窗口内请求 · 峰值（条/秒）· 累计
  （累计语义：自 chronyd 启动，重启清零，卡片标题已注明）。
- 图 2「各网口请求量」：uPlot bars（eth0-3 分类轴，累计值）；tooltip 显示累计 + 所选窗口增量
  （1m/1h/5h/24h，随页级时间范围切换；1m 口径为最近一个采样间隔，扫描边界 90s 容忍采样抖动）。
- 页级时间范围（近 1 分钟/1 小时/5 小时/24 小时，默认 1 分钟）为**纯本地切片**，
  同时联动：图 1 切片、图 2 tooltip、授时服务状态表的 req/valid/rsp 窗口字段、
  客户端明细的 last_age_s 过滤与说明文案。1 分钟窗口在趋势图上只有 1-2 个点
  （CSV 每分钟一个样本，数据粒度使然）。
- **1 分钟窗口的秒级数据**：`ntp_nic_monitor` 在收包路径维护每网口**逐秒请求环**
  （120 秒），快照 JSON 以 `"sec_t0"` + 每网口 `"sec":[...]` 发布（idle/断档秒为 0）。
  1m 视图优先切该环的尾部 60 秒直接按 条/秒 绘制；tooltip 时间带秒
  （如 09-19 11:42:01）、"每秒 X 条"打头；峰值卡为真实秒级峰值。旧快照缺
  `sec` 字段时回退 CSV ÷60 的分钟近似。1m 模式下页面轮询加快到 3 秒
  （`schedNtpMon` 自适应），守护进程快照间隔相应为 1 秒（`-i 1`）。
